...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if not self.shared_memory_attention:
            ...
            _,_, self.memory, out_hx_mem_new, attention_x = self.relational_memory(
                 inputs=key,
                 memory=self.memory,  #.cuda(),
                 encoder_padding_mask=key_padding_mask  # # TODO: encoder_padding_mask
             )
            #print('time taken by relational:' + str(time.time() - t2))

            return out_hx_mem_new.transpose(0, 1), self.memory, attention_x
